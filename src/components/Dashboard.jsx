  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const occupiedKennels = analyticsData.filter(
          (record) => record.start_date <= new Date() && record.end_date >= new Date()
        );
        const totalKennels = analyticsData.length;
        const occupancyRate = ((occupiedKennels.length / totalKennels) * 100).toFixed(2);

        // Calculate Average Stay Duration
        const totalDaysstayed = analyticsData.reduce(
          (acc, record) => acc + record.days_stayed,
          0
        );
        const avgStayDuration = (totalDaysstayed / analyticsData.length).toFixed(2);

        // Calculate Customer Return Rate
        const uniqueCustomers = new Set(analyticsData.map((record) => record.customer_id));
        const customerReturnRate = ((uniqueCustomers.size / analyticsData.length) * 100).toFixed(2);

        setOccupancyRate(occupancyRate);
        setAvgStayDuration(avgStayDuration);
        setCustomerReturnRate(customerReturnRate);

        // Calculate monthly and yearly occupancy rates (existing code)
        // ...
      } catch (err) {
        setError(err.message); // Handle error
      } finally {
